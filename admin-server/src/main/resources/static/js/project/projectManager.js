var app = new Vue({
    ...
    methods: {
        ...
        showCreateDialog() {
            ...
            this.createUrl = 'projectCreate.html?new=' + Math.random()
        },
        showModifyDialog(index, row) {
            this.modifyDialog = true
            this.modifyUrl = 'projectModify.html?new=' + Math.random()
        ...
        handleDelete(index, row) {
            ...
            this.$confirm('确认删除该项目？', '提示', {
                ...
                Vue.http.post('../../project/delete/' + row.id,
                    {
                    }
                ).then(function (res) {
                    var data = res.body;
                    if (data.status === 0) {
                        self.$message.success("删除项目成功")
                        self.fetchData()
                    } else {
                        self.$message.error(data.statusInfo)
                    }
                }, function () {
                    self.$message.error('删除项目失败');
                });
            }, function () {
                return
            })
        ...
        auth: function () {
            var authCode = {
                ...
                'admin.project.search': 'search',
                'admin.project.modify': 'modify',
